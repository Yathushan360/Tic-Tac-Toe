...
int main() {
    ...
    // 3 & 4. Display Confirmation and Rules
    displayRules(N);

    // 5. Select Game Mode
    gameMode = selectGameMode();

    // Initialize the board and clear the log file for a new game
    initializeBoard(board, N);
    FILE *file = fopen("tictactoe_log.txt", "w");
    if (file != NULL) {
        fprintf(file, "--- New Game Log ---\n");
        fprintf(file, "Board Size: %dx%d\n", N, N);
    ...
    while (!gameOver) {
        ...
        // Determine the current player and their symbol
        if (gameMode == 3) { // Three-player mode
        ...
        // Get the move based on the game mode and current player
        if (gameMode == 2 && currentPlayer == 2) {
            // Computer's turn
            printf("\nPlayer 2 (Computer) is making a move...\n");
        ...
        // If the game is not over, switch to the next player
            ...
            if (gameMode == 3) {
                currentPlayer = (currentPlayer % 3) + 1; // Switches players 1 -> 2 -> 3 -> 1
            } else {
                currentPlayer = (currentPlayer == 1) ? 2 : 1; // Switches players 1 -> 2 -> 1
            }
        }
    }
 ...
 */
void logMoveToFile(int player, int row, int col, char symbol) {
    FILE *file = fopen("tictactoe_log.txt", "a"); // "a" for append mode
    if (file == NULL) {
